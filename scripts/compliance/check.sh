...
# Canonical pattern: REUSE (CLI + CI) + OpenSSF automated gates (IVD Customer #0).
#
...
headers = {"User-Agent": "horizon-trustlint-bootstrap"}
if token:
...
while IFS= read -r -d '' f; do
  ...
  check_file "$f"
done < <(find docs/spec docs/cursor-rules -type f \( -name '*_intent.yaml' -o -name '*.mdc' \) -print0 2>/dev/null || true)

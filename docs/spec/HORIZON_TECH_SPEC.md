...
**Based on**: [PRD](./THCP_FIDELITY_MONITOR_PRD.md) · [Intent](./horizon_intent.yaml)
...
> **"Spacetime" is a metaphor.** The relativity vocabulary (proper time, Minkowski interval, light
> cone) is design inspiration, not a physical claim. Every signal in this section reduces to standard
> arithmetic on embeddings and timestamps — cosine distances, an exponential-decay retention term, a
> circadian lookup, and weighted sums. The labels are descriptive, not theoretical evidence.
...
The `spacetime_interval` (ds²) is a **weighted 4-term distance**, and its `interval_class` is just the
sign bucket of that distance. The Minkowski signature is a naming convention — the minus sign on the
time term is arbitrary and flipping it flips the class. **This signal is descriptive metadata only: no
event or fidelity score consumes `interval_class`.** It is kept because the class label is occasionally
useful for human inspection, not because it adds predictive power.
...
> **"Light cone" is a metaphor.** This section computes how many prior turns are still *live* —
> in-window, not yet memory-decayed, and topically related — as a product of three [0, 1] factors
> thresholded at θ_R. There is no physics here; the name just describes the "what can still be
> referenced" intuition.
